      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-gray-400 tracking-widest ml-1 mt-1">SEISEI</span>
          </div>
      ...
      <section className="relative pt-28 pb-8 px-6">
        ...
        <div className="relative max-w-7xl mx-auto">
          ...
          <div className="text-center mb-16">
            <div className="inline-flex items-center gap-2 bg-gradient-to-r from-fuchsia-50 to-violet-50 border border-fuchsia-100 px-4 py-1.5 rounded-full mb-6">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-fuchsia-400 opacity-75" />
                <span className="relative inline-flex rounded-full h-2 w-2 bg-fuchsia-500" />
              </span>
              <span className="text-xs font-semibold text-fuchsia-700">Segmind AI 搭載</span>
            </div>
            ...
            <h1 className="text-5xl md:text-7xl font-black tracking-tight text-gray-900 leading-[1.05]">
              ...
              を<br className="hidden md:block" />自動化する
            </h1>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mt-8">
              <Link
                href="/signup"
                className="inline-flex items-center gap-2 bg-black text-white h-12 px-7 rounded-xl text-sm font-bold hover:bg-gray-800 transition-all shadow-lg shadow-black/10 hover:shadow-xl hover:shadow-black/20"
              >
                無料で始める
                <ArrowRight className="w-4 h-4" />
              </Link>
              <button className="inline-flex items-center gap-2 text-gray-500 h-12 px-6 rounded-xl text-sm font-medium hover:text-black hover:bg-gray-50 transition-colors border border-gray-200">
                <Play className="w-4 h-4" />
                デモを見る
              </button>
            </div>
            <p className="text-xs text-gray-300 mt-4">クレジットカード不要 · 10枚まで無料生成</p>
          </div>

          {/* ====== VISUAL FLOW — Input → AI → Outputs ====== */}
          <div className="relative max-w-6xl mx-auto">

            {/* SVG connection lines (dashed, animated) */}
            <svg
            ...
            >
              {/* Input image → Center output */}
              <path
                d="M 220 260 C 400 260, 350 180, 600 180"
                stroke="url(#grad1)"
                strokeWidth="1.5"
                strokeDasharray="6 4"
                className="animate-dash"
              />
              {/* Garment → Center output */}
              <path
                d="M 220 420 C 400 420, 400 320, 600 260"
                stroke="url(#grad1)"
                strokeWidth="1.5"
                strokeDasharray="6 4"
                className="animate-dash"
              ...
              {/* Center → Right output */}
              <path
                d="M 740 200 C 850 200, 850 260, 980 260"
                stroke="url(#grad2)"
                strokeWidth="1.5"
                strokeDasharray="6 4"
                className="animate-dash"
              ...
              {/* Center → Right bottom */}
              <path
                d="M 740 320 C 850 320, 850 420, 980 420"
                stroke="url(#grad2)"
                strokeWidth="1.5"
                strokeDasharray="6 4"
                className="animate-dash"
            ...
            <div className="flex items-center justify-center gap-4 mt-8">
              {['SegFit v1.3', 'Z-Image Turbo', 'FaceSwap v5'].map((tech) => (
                <span key={tech} className="inline-flex items-center gap-1.5 bg-gray-50 border border-gray-100 text-[10px] font-semibold text-gray-400 px-3 py-1.5 rounded-full">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
                  {tech}
                </span>
              ))}
